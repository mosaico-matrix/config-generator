...
class MosaicoDeviceState with ChangeNotifier {
  ...
  bool get isConnecting => _isConnecting ?? true; // But we assume that we are connecting from outside

  /// We only need to know if the device is connected to COAP, BLE is secondary
  bool get isConnected => _isCoapConnected ?? false;  // At first we don't know if COAP is connected so we assume it's not
  String get coapConnectionStatusText => isConnected ? 'Connected' : 'Disconnected';

  /// BLE connection status
  String get bleConnectionStatusText => _isBleConnected == true ? 'BLE available' : 'BLE not available';
  ...
  Future<String> _getMatrixIp() async
  {
    ...
    if(_matrixIp != null && await _pingMatrix(_matrixIp!))
      return _matrixIp!;
